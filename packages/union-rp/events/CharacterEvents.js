...
module.exports = {
    ...
    "authCharacter": (player, characterIndex) => {
        ...
        if (character.promoId !== 0) {
            ...
            //console.log("За персонажем закреплен промо-код! уровень = ",characterLevel);
    ...
    "characters.openSlot": (player, type) => {
        ...
        if (type == "achievements") {
            ...
            if (player.account.achievements_slot) return player.utils.error(`Слот за достижения уже открыт!`);
            if (count < achievements) return player.utils.error(`Необходимо открыть достижения! (${count} из ${achievements})`);
            DB.Handle.query(`UPDATE accounts SET achievements_slot=? WHERE id=?`,
                [1, player.account.id]);
            player.account.achievements_slot = true;
        } else if (type == "donate") {
            if (!player.account.achievements_slot) return player.utils.error(`Сначала разблокируйте слот за достижения!`);
...
function initPlayerUtils(player) {
    ...
    player.utils.setBankMoney = (newMoney) => {
        ...
        mp.logs.addLog(`Игроку ${player.name} было назначено с ${player.bank} на ${newMoney} банковских денег`, 'main', player.account.id, player.sqlId, { oldMoney: player.money, newMoney: newMoney });
    ...
    player.utils.setMoney = (newMoney, dontTouchInventory, callback) => {
        newMoney = Math.clamp(newMoney, 0, Number.MAX_VALUE);
        mp.logs.addLog(`Игроку ${player.name} было назначено с ${player.money} на ${newMoney} наличных денег`, 'main', player.account.id, player.sqlId, { oldMoney: player.money, newMoney: newMoney });
    ...
    player.utils.bank = (title, text) => {
        player.call("BN_ShowWithPicture", [title, "Банк", text, "CHAR_BANK_MAZE", 2]);
    };
    ...
    player.utils.setWanted = (newWanted) => {
        ...
        player.wantedTimer = timerId = setInterval(() => {
            try {
                ...
                if (!rec.wanted) rec.utils.info(`Ваш уровень розыска очищен!`);
        ...
        mp.players.forEach((rec) => {
            if (rec.admin > 0 && rec.load == 1) {
                if(event == 'setNewReport') {
                    ...
                    rec.utils.warning(`Поступил новый тикет...`);
                } else if(event == 'setNewMessage') {
...
function initLocalParams(player) {
    ...
    mp.logs.addLog(`${player.name}[${player.id}] авторизовался на сервере. IP: ${player.ip}, socialClub: ${player.socialClub}`, 'main', player.account.id, player.sqlId, { socialClub: player.socialClub, id: player.id, ip: player.ip });
}
