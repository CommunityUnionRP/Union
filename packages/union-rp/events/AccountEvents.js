...
module.exports = {
    ...
    "closedMode.open": (player, pin) => {
        ...
        if (pin != Config.pin) {
            ...
            mp.logs.addLog(`${player.name} ввел неверно PIN! - ${pin}<br />IP: ${player.ip}<br /> Social Club: ${player.socialClub}`, 'main', player.account.id, player.sqlId, { pin: pin, ip: player.ip, socialClub: player.socialClub });
            player.pinCount++;
