...
module.exports = {
    Init: () => {
      DB.Handle.query("SELECT * FROM phone_special", (e, result) => {
        ...
        console.log(`[Номера] Загружено ${result.length} специальных номеров`);
      });
      DB.Handle.query("SELECT * FROM phone_taken", (e, result) => {
        ...
        console.log(`[Номера] Загружено ${PhoneControl.free_numbers.length} свободных номеров`);
      });
    }
}
...
global.initPlayerTelephone = function(player) {
    ...
    for (let i = 0; i < array.length; i++) {
      if (array[i].num == num && array[i].id != id) player.utils.error("У вас уже есть контакт с таким номером!");
        ...
        DB.Handle.query("SELECT * FROM characters WHERE phone=?", [num], (e, result) => {
             ...
             player.utils.info("Вы отредактировали контакт!");
             return;
        });
      }
    }
  ...
  DB.Handle.query(`SELECT * FROM phone_contacts WHERE creator=?`, [player.sqlId], (e, result) => {
		if (e) {
			console.log(`Контакты телефона не загружены для персонажа '${player.sqlId}'. ${e}`);
			return;
		}
