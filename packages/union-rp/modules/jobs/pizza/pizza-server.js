...
const PizzaJob = {
  ...
  functions: {
    ...
    joinPizzaJob(player) {
      ...
      player.utils.info("Возьмите пиццу со склада!");
    ...
    takePizzaFromStorage(player) {
      if (player.havepizza > 0) {
        player.utils.error("Сначала развезите предыдущию пиццу!");
        return;
...
mp.events.add("give.pizza.item", (player, i) => {
    try
    {
      ...
      if (player.havepizza < 1) {
        player.utils.warning("У вас недостаточно пиццы!");
        return;
      }
      ...
      player.havepizza--;
      if (player.havepizza < 1) {
        player.utils.warning("У вас закончилась пицца, направляйтесь на склад!");
