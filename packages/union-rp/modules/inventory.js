module.exports.Init = function() {
    ...
    DB.Handle.query("SELECT * FROM inventory_items", (e, result) => {
        ...
        console.log(`Предметы инвентаря загружены: ${i} шт.`);
    });
...
global.initPlayerInventory = function(player) {
    ...
    player.inventory.add = (itemId, params, items, callback, dontTouchHandlers) => {
        ...
        if (!mp.inventory.getItem(itemId)) return callback(`Неверный ITEM ID: ${itemId} предмета!`);
        var freeSlot = player.inventory.findFreeSlot(itemId);
        if (!freeSlot) return callback(`Свободный слот для предмета не найден! Предмет: ${mp.inventory.getItem(itemId).name}`);
        ...
        if (weight + mp.inventory.items[itemId - 1].weight > Config.maxInventoryWeight) return callback("Недостаточно выносливости!");
        ...
        if (item.params.weaponHash) {
            ...
            if (alreadyExists) return callback(`Оружие ${mp.inventory.getItem(item.itemId).name} уже имеется!`);
    ...
    player.inventory.addToItem = (item, parentSqlId, index, callback, dontTouchHandlers) => {
        ...
        var weight = player.inventory.getCommonWeight();
        if (weight + mp.inventory.items[item.itemId - 1].weight > Config.maxInventoryWeight) return callback("Недостаточно выносливости!");
    ...
    player.inventory.addOld = (sqlId, item, callback = () => {}, dontTouchHandlers) => {
        ...
        if (!mp.inventory.getItem(item.itemId)) return callback("Неверный ID предмета!");
        ...
        if (item.items) {
            for (var key in item.items) {
                ...
                if (child.params.weaponHash) {
                    ...
                    // if (alreadyExists) return callback(`Оружие ${mp.inventory.getItem(child.itemId).name} уже имеется!`);
                ...
                if (child.items) {
                    for (var key2 in child.items) {
                        ...
                        if (child2.params.weaponHash) {
                            ...
                            // TODO: Придумать что-то с проверкой этой.
                            // if (alreadyExists) return callback(`Оружие ${mp.inventory.getItem(child2.itemId).name} уже имеется!`);
    ...
    player.inventory.deleteByParams = (itemId, keys, values, callback) => {
        ...
        if (keys.length != values.length) return callback(`Количество ключей и значений параметров предмета не совпадает!<br/>${player.name}<br/>keys: ${keys}<br/>values: ${values}`);
...
global.initVehicleInventory = function(vehicle) {
    ...
    vehicle.inventory.add = (itemId, params, items, callback, dontTouchHandlers, index = -1) => {
        ...
        if (index == -1) return callback(`Свободный слот для предмета не найден! Предмет: ${mp.inventory.getItem(itemId).name}`);
        //debug(`Свободный слот: ${JSON.stringify(freeSlot)}`) // неверный freeSlot.parentSqlId - bug #42

        var weight = vehicle.inventory.getCommonWeight();
        if (weight + mp.inventory.items[itemId - 1].weight > Config.maxVehInventoryWeight) return callback("Недостаточно грузоподъемности!");
    ...
    vehicle.inventory.addOld = (sqlId, item, callback = () => {}, dontTouchHandlers) => {
        ...
        if (!freeSlot) return callback("Свободный слот для предмета не найден!");
        var weight = vehicle.inventory.getCommonWeight();
        if (weight + mp.inventory.items[item.itemId - 1].weight > Config.maxVehInventoryWeight) return callback("Недостаточно грузоподъемности!");
