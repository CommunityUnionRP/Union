...
function loadFarmsFromDB() {
    ...
    DB.Handle.query("SELECT * FROM farms", (e, result) => {
        ...
        console.log(`Фермы загружены: ${i} шт.`);
    });
    initFarmsUtils();
}
...
function loadFarmFieldsFromDB() {
    ...
    DB.Handle.query("SELECT * FROM farms_fields", (e, result) => {
        ...
        console.log(`Поля фермы загружены: ${i} шт.`);
    });
    initFarmFieldsUtils();
}
...
function initFarmUtils(farm) {
    ...
    farm.sellToGos = () => {
        ...
        mp.log(`Ферма ${farm.sqlId} продана в гос!`);
    }
...
function initFarmFieldUtils(field) {
    ...
    field.startFilling = (player, vehicle) => {
        ...
        player.utils.info(`Следуйте к метке для начала посева!`);
    };
...
function initFarmLabels(farm) {
    ...
    pos.z++;
    farm.label = mp.labels.new(`~y~Ферма ~w~№${farm.sqlId}\n ~b~Pay: ~w~${farm.pay}$`, pos, {
        los: true,
        font: 4,
        drawDistance: 30,
        color: [0, 187, 255, 255],
    ...
    farm.pumpkinLabel = mp.labels.new(`~y~Тыква: ~w~${farm.pumpkins} из 2000 ед.\n~g~Цена: ~w~${farm.pumpkinPrice}$`, positions[0], {
        los: false,
        font: 4,
        drawDistance: 30,
        color: [0, 187, 255, 255],
    ...
    farm.cabbageLabel = mp.labels.new(`~y~Капуста: ~w~${farm.cabbages} из 2000 ед.\n~g~Цена: ~w~${farm.cabbagePrice}$`, positions[1], {
        los: false,
        font: 4,
        drawDistance: 30,
        color: [0, 187, 255, 255],
    ...
    farm.weedLabel = mp.labels.new(`~y~Травка: ~w~${farm.weeds} из 2000 ед.\n~g~Цена: ~w~${farm.weedPrice}$`, positions[2], {
        los: false,
        font: 4,
        drawDistance: 30,
        color: [0, 187, 255, 255],
    ...
    farm.grainLabel = mp.labels.new(`~b~Прием зерна: ~w~${farm.grains} из 8000 ед.\n~g~Цена: ~w~${farm.grainPrice}$`, positions[3], {
        los: false,
        font: 4,
        drawDistance: 30,
        color: [0, 187, 255, 255],
