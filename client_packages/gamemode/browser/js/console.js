    ...
    window.consoleAPI = {
        ...
        removeReport: (sqlId) => {
            ...
            if (!$("#console .report-list .report").length) {
                $("#console .report-list .content").append(`<div class="empty">Можно отдохнуть!</div>`);
            }
            if ($("#console .report-dialog").data("sqlid") == sqlId) {
                $("#console .report-dialog").data("sqlid", 0);
                $("#console .dialog-messages").empty();
                $("#console .dialog-messages").append(`<div class="empty">Тикет закрыт!</div>`);
            }
        ...
        addReportMessage: (sqlId, messages) => {
            ...
            var ownerName = report.messages[0].name || "Игрок";
            if (name != ownerName && ownerName != "Игрок") text = `[А]: ${text}`;
        ...
        sendMessage: (message) => {
            ...
            if (!reportId) return nError(`ID тикета не найден!`);
        ...
        showReportMenu: (sqlId, left, top) => {
            ...
            var menu = ["Закреп", "Закрыть", "ТП к игроку", "ТП к себе"];
