$(document).ready(function() {
    ...
    function initPassport(data) {
        var relationship = (data.sex == 1) ? `Женат на ${data.relationshipName}` : `Замужем за ${data.relationshipName}`;
        if (!data.relationshipName) relationship = (data.sex == 1) ? `Не женат` : `Не замужем`;
        data.sex = (data.sex == 1) ? "Мужской" : "Женский";
        var age = convertMinutesToLevelRest(data.minutes).level;
        data.houses = (data.houses.length == 0) ? "Не имеется" : `${data.houses}`;
    ...
    function initSelectorButtons() {
        ...
        $('#licenses_text').on("click", function() {
            if ($('#licenses_text').text().trim() === "Развернуть страницу...") {
                $(licenses).css({
                ...
                $('#licenses_text').text('Свернуть страницу...');
                $('.fishing').css({
                    'display': 'block'
                });
            } else {
                ...
                $('#licenses_text').text('Развернуть страницу...');
                $('.fishing').css({
                    'display': 'none'
                });
            }
