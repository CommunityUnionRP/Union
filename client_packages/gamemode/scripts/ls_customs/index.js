...
function showHornsMenu() {
	...
	if (currentHornType === 0) {
		mp.events.call("selectMenu.setHeader", "ls_customs_hornsConcrete", "Клаксоны: Стандартные");
	} else if (currentHornType === 1) {
		mp.events.call("selectMenu.setHeader", "ls_customs_hornsConcrete", "Клаксоны: Музыкальные");
	} else {
		mp.events.call("selectMenu.setHeader", "ls_customs_hornsConcrete", "Клаксоны: С повтором");
	}
...
function showModificationSetMessage(itemName) {
	mp.game.ui.notifications.show(`Модификация ~g~${itemName}~w~ успешно установлена`, true);
