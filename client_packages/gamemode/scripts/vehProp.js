...
exports = (menu) => {
    ...
    mp.events.add('vehicleEngineHandler', () => {
        ...
        if (!engine) {
            ...
            if (player.vehicle.fuel <= 0) return mp.events.call(`nError`, "Нет топлива!");
            if (engineBroken) return mp.events.call(`nError`, "Двигатель поломан!");
            if (oilBroken) return mp.events.call(`nError`, "Залейте масло!");
            if (accumulatorBroken) return mp.events.call(`nError`, "Аккумулятор поломан!");
        }
