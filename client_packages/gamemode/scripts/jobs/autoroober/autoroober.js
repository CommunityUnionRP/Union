...
let rooberVehColShape = mp.colshapes.newSphere(126.815, -3105.666, 5.595, 4); // Колшейп для сдачи угнанного ТС
...
mp.events.add('createRooberPlace', (x, y, z, sec, vname, veh, givemoneys) => {
	try
	{
		...
		mp.game.ui.notifications.showWithPicture("Сообщение", "Симон", "Ты должен найти ~b~" + vname + " ~w~и привезти его в порт!", "CHAR_SIMEON", 2);
		time = sec;
		vehic = veh;
		vehname = vname;
		gmoney = givemoneys;
		timeBar = new timerBarLib.TimerBar("ОСТАЛОСЬ");
		gtimer = new timerBarLib.TimerBar("ПРИБЫЛЬ");
		...
		timer = setInterval(function() {
		   if (time > -1) {
			   ...
			   if (vehic === undefined) {
				   mp.events.callRemote("destroy.roober.place");
					 mp.game.ui.notifications.showWithPicture("Сообщение", "Симон", "~r~Вы провалили задание!", "CHAR_SIMEON", 2);
				   return;
			   }
			    ...
			    if (find == false)  {
				     ...
				     if (dist < 10.0) {
					      mp.game.graphics.notify("Вы обнаружили ~b~" + vehname);
					      mp.game.ui.removeBlip(blipz);
					      blipz = null;
					      blipz = mp.blips.new(669, new mp.Vector3(x, y, z), { alpha: 255, color: 74, name: vehname });
				        blipz2 = mp.blips.new(473, new mp.Vector3(126.815, -3105.666, 5.595), { alpha: 255, color: 74, scale: 0.8, name: "Склад" });
	     ...
	     if (hack > 0) {
			 ...
			 if (hack == 0) {
				 ...
				 mp.game.graphics.notify("~r~Вы закончили взлом транспорта!");
...
mp.events.add('startRoobingVehicle', (time) => {
  ...
  mp.players.local.freezePosition(true);
	...
	mp.game.graphics.notify("~r~Вы начали взлом транспорта!");
	hacktimer = new timerBarLib.TimerBar("ВЗЛОМ", true);
...
mp.events.add('render', () => {
   if (find == true) {
	   ...
	   mp.game.graphics.drawText(`Доберитесь до ~b~места ликвидации транспорта`, [0.5, 0.95], { scale: 0.9, color: [255, 255, 255, 255], font: 4, outline: true });
    } else if (find == false && blipz != undefined) {
	  mp.game.graphics.drawText(`Угоните ~b~` + vehname, [0.5, 0.95], { scale: 0.9, color: [255, 255, 255, 255], font: 4, outline: true });
    }
});
...
mp.events.add('playerEnterColshape', (shape) => {
    if (shape === rooberVehColShape) {
		if (mp.players.local.vehicle && vehic !== undefined) {
			if (mp.players.local.vehicle == vehic) {
				mp.events.call("ShowMidsizedShardMessage", "УГОН ЗАВЕРШЕН", "Вы заработали ~g~$" + gmoney, 2, false, true, 6000);
