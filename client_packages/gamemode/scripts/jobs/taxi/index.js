...
mp.events.add('clear.taxi.filtration', (type) => {
  JobTaxi.blacklist = [];
  mp.game.ui.notifications.showWithPicture('Информация', 'Downtown Cab Co.', 'Вы ~y~сбросили ~w~фильтрацию заказов.', 'CHAR_TAXI');
});
mp.events.add('add.taxi.filtration', (name) => {
  if (!JobTaxi.blacklist.includes(name)) {
    JobTaxi.blacklist.push(name);
    mp.game.ui.notifications.showWithPicture('Информация', 'Downtown Cab Co.',  '~y~' + name + ' ~w~добавлен в фильтрацию и не будет отображаться.', 'CHAR_TAXI');
  }
});
...
mp.events.add('cant.finish.taxi.job', () => {
  mp.game.ui.notifications.showWithPicture('Информация', 'Downtown Cab Co.', 'Вы ~y~не завершили ~w~вызов.', 'CHAR_TAXI');
});
...
mp.events.add('get.taxi.waypoint', (target) => {
   ...
   if (pos === undefined) {
     mp.game.ui.notifications.showWithPicture('Информация', 'Downtown Cab Co.', 'Вы ~y~не установили ~w~метку на карте.', 'CHAR_TAXI');
     return;
   }
...
mp.keys.bind(0x45, false, function () { // E key
	if (keyDownE !== undefined) {
      ...
      if (keyDownE === true) {
          mp.events.call("choiceMenu.show", "accept_job_taxi", {name: "уволиться из Таксопарка?"});
      } else {
					mp.events.call("choiceMenu.show", "accept_job_taxi", {name: "устроиться в Таксопарк?"});
      }
		}
	}
