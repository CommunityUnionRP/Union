...
mp.events.add('create.job.builder.mark', (posx, posy, posz, type) => {
    ...
    deleteData();
		if (type === true) {
			JobBuilder.blip = mp.blips.new(1, new mp.Vector3(posx, posy), { alpha: 255, color: 1, name: "Пункт назначения" });
...
mp.events.add('create.job.builder.load', () => {
    ...
    deleteData();
		...
		JobBuilder.blip = mp.blips.new(1, new mp.Vector3(bonePos.x, bonePos.y), { alpha: 255, color: 1, name: "Пункт назначения" });
...
mp.events.add('playerEnterColshape', (shape) => {
    if (shape === oshape) mp.events.call("prompt.show", `Нажмите <span>Е</span> для взаимодействия`);
