...
mp.keys.bind(0x45, false, function () { // E key
	if (JobTrash.keyDownE !== undefined) {
      ...
      if (JobTrash.keyDownE === true) {
          mp.events.call("choiceMenu.show", "accept_job_trash", {name: "уволиться из Центра утилизации?"});
      } else {
					mp.events.call("choiceMenu.show", "accept_job_trash", {name: "устроиться в Центр утилизации?"});
      }
		}
	}
...
mp.events.add('createPlaceTrasher', (x, y, z, status) => {
  ...
  if (status === true) {
    ...
    JobTrash.blip = mp.blips.new(1, new mp.Vector3(x, y), { alpha: 255, color: 3, name: "Мусорный бак" });
...
mp.events.add('update.trash.vehicle', (vehicle, min, max) => {
  try {
    ...
    if (!JobTrash.trashinfo) JobTrash.trashinfo = new timerBarLib.TimerBar("ВМЕСТИМОСТЬ");
...
mp.events.add('playerEnterColshape', (shape) => {
   if (shape === JobTrash.colshape && !mp.players.local.vehicle && JobTrash.jobtype !== 1) {
     if (JobTrash.count >= 30) {
       mp.events.call("nWarning", "Ваш транспорт переполнен, выгрузите мусор на свалку!");
       return;
     }
     JobTrash.jobtype = 1;
     ...
     setTimeout(() => {
       mp.players.local.freezePosition(false);
       mp.events.call("nWarning", "Положите мешок в задний сектор мусоровоза!");
       mp.events.call("prompt.show", `Нажмите <span>E</span>, чтобы положить мешок в задний сектор!`);
