...
mp.events.add('delete.gopostal.colshape', (i, type) => {
  ...
  if (type === 0) {
    ...
    PostalJob.blip = mp.blips.new(615, new mp.Vector3(78.92, 112.55, 81.17), { alpha: 255, scale: 1.0, color: 2, name: "Склад Go Postal", shortRange: false });
    PostalJob.blip.setRoute(true);
    PostalJob.blip.setRouteColour(2);
    mp.events.call("prompt.show", `Направляйтесь на склад и возьмите заказ, после чего загрузите фургон.`);
  }
});
...
mp.events.add('playerEnterColshape', (shape) => {
    if (shape === PostalJob.colshape) {
      ...
      mp.events.call("prompt.show", `Нажмите <span>Е</span> для того, чтобы взять заказ!`);
      PostalJob.keyDownE = true;
    } else if (shape === PostalJob.storage.shape || shape == PostalJob.storage.oshape) {
       if (mp.players.local.vehicle) {
         mp.events.call("nError", "Выйдите из транспортного средства!");
       } else {
         if (!PostalJob.vehicle) {
            mp.events.call("nError", "У вас нет фургона, чтобы загрузить почту!");
            return;
         }
...
mp.events.add('clear.storage.gopostal', () => {
  ...
  for (let i = 0; i < arr.length; i++) {
      PostalJob.blips.push(mp.blips.new(1, new mp.Vector3(arr[i].x, arr[i].y), { alpha: 255, scale: 1.1, name: "Точка сдачи почты", color: 3 }));
...
mp.events.add('control.gopostal.blip', (num) => {
  try
  {
    ...
    switch (num) {
      case 1:
        PostalJob.blip = mp.blips.new(501, new mp.Vector3(-321.17, -887.45, 31.07), { alpha: 255, scale: 1.0, color: 2, name: "Парковка Go Postal", shortRange: false });
        mp.events.call("prompt.show", `Проследуйте на парковку и возьмите рабочий фургон.`);
        break;
      case 2:
        PostalJob.blip = mp.blips.new(615, new mp.Vector3(78.92, 112.55, 81.17), { alpha: 255, scale: 1.0, color: 2, name: "Склад Go Postal", shortRange: false });
        PostalJob.blip.setRoute(true);
        ...
        break;
      case 3:
        PostalJob.blip = mp.blips.new(615, new mp.Vector3(69.25, 127.65, 79.21), { alpha: 255, scale: 1.0, color: 1, name: "Склад Go Postal", shortRange: false });
        mp.events.call("prompt.show", `У вас есть 5 минут, чтобы загрузить почту в фургон.`);
    }
  }
  catch (err) {
      mp.game.graphics.notify(err);
...
mp.keys.bind(0x45, false, function () { // E key
	if (PostalJob.keyDownE !== undefined) {
      ...
      if (PostalJob.keyDownE === true) {
          mp.events.call("choiceMenu.show", "accept_job_postal", {name: "уволиться из Go Postal?"});
      } else {
					mp.events.call("choiceMenu.show", "accept_job_postal", {name: "устроиться в Go Postal?"});
      }
