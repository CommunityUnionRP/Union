...
mp.events.add('admin.get.boneindex', (boneName) => {
    if (mp.players.local.vehicle) {
        ...
        if (boneid === -1) return mp.events.call("console.push", `error`, `У вашего транспорта нет кости "${boneName}"`);
        mp.events.call("console.push", `info`, `Индекс кости у транспорта "${boneName}" - ${boneid}`);
    } else {
        let boneid = mp.players.local.getBoneIndexByName(boneName);
        if (boneid === -1) return mp.events.call("console.push", `error`, `У вашего игрока нет кости "${boneName}"`);
        mp.events.call("console.push", `info`, `Индекс кости у игрока "${boneName}" - ${boneid}`);
    }
});
...
mp.events.add('render', () => {
    ...
    if (mp.clientStorage["demorganSet"]) {
        ...
        mp.game.graphics.drawText(`Деморган: ~r~${(mp.clientStorage["demorganSet"].demorgan + Math.ceil(diff / 60)).toFixed(0)} минут`, [0.5, 0.96], {
            scale: 0.6,
